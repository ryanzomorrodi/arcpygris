...
def download(geom_type: str, year: str, output_path: str, **kargs):
    ...
    if ".gdb" in output_path.lower():
        arcpy_helpers.add_metadata(
            output_path,
            geography = "Alaska Native Regional Corporation",
            year = year,
            req_urls = [req_url],
            geom_type = geom_type
        )
...
def get_url(geom_type: str, year: str) -> str:
    ...
    if geom_type == "Cartographic":
        if year_int == 2013:
            req_url = f"https://www2.census.gov/geo/tiger/GENZ{year}/cb_{year}_02_anrc_500k.zip"
        elif year_int >= 2014:
            req_url = f"https://www2.census.gov/geo/tiger/GENZ{year}/shp/cb_{year}_02_anrc_500k.zip"
    else:
        if year_int >= 2013:
            req_url = f"https://www2.census.gov/geo/tiger/TIGER{year}/ANRC/tl_{year}_02_anrc.zip"
